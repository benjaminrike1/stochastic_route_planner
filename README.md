...
The notebook folder consists of three notebook: final\_project, predictive\_modelling, and data\_pipeline. Predictive\_modelling and data\_pipeline covers what their names suggest, but are not needed to run the final project. All the important work from the notebooks are saved in hdfs and loaded back from there. To run the application, you should focus on final_project.ipynb. By running all the cells in the notebook, you should be able to interact with the application and test it. Especially the user interface found in the last section, Application, can be used to display different routes. More information can be found in the top of the notebook.

Some other things to keep in mind:
- The session must be started with 2GB or more memory. (uses approx. 1.80 GB)
- Our video presentation can be found here: https://moodle.epfl.ch/mod/assign/view.php?id=1152777
- All files are written to /group/gutane/, so you need read permission in this folder to load the data in final_project.ipynb
